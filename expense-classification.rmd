...
# Change in expense categories

```{r delta, echo = FALSE}
amounts_subcat %>%
  filter(year_month == unique(.$year_month)[2:3]) %>%
  ...
  gt() # %>%
  # tab_style(cell_fill("black"), cells_column_labels()) %>%
  # tab_style(cell_text("white", weight = "bold"), cells_column_labels())
```
...
records %>%
  ...
  gt() %>%
  tab_style(cell_fill("goldenrod"), cells_row_groups()) %>%
